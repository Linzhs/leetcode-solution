...
func isSymmetricUsingRecursive(node1 *pkg.TreeNode, node2 *pkg.TreeNode) bool {
...
func isSymmetricUsingIterative(root *pkg.TreeNode) bool {
	if root == nil {
		return true
	}

	queue := []*pkg.TreeNode{root, root}
	for len(queue) != 0 {
		n1 := queue[0]
		n2 := queue[1]
		queue = queue[2:]
